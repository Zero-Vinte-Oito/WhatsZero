          ...
          <div class="q-pa-sm">
            <q-card class="bg-white btn-rounded" style="width: 100%" bordered flat>
              <q-card-section class="text-center">
                ...
                <!-- <div class="text-caption q-mt-md" style="font-size: 14px" id="email" @click="copyContent(ticketFocado.contact.email || '')">
                  {{ ticketFocado.contact.email || '' }}
                </div> -->
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat>
              ...
              <q-card-section class="q-pa-sm">
                <q-item>
                  <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
                    <!-- <q-btn flat class="btn-outline btn-small" label="Registrar" icon="mdi-send-check" @click="notaEdicao = {}; modalNota = true" /> -->
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="ticketFocado.id + $uuid()"  v-if="dialogflowAtivo === 'enabled'">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
...
import { ConsultarDadosTicket, ConsultarLogsTicket, ConsultarTickets, DeletarMensagem, AtualizarTypebotTicket, AtualizarDialogflowTicket, AtualizarChatgptTicket, EnviarMensagemTexto, AtualizarStatusTicket } from 'src/service/tickets'
...
export default {
  ...
  computed: {
    ...
    openTickets() {
      ...
      // const consultPromises = [];
      // const ticketNovos = [];
      // for (const tick of filteredTickets) {
      //   const newTicketPromise = ConsultarDadosTicket(tick);
      //   consultPromises.push(newTicketPromise);
      //   newTicketPromise.then(newTicket => {
      //     //console.log(newTicket.data);
      //     ticketNovos.push(newTicket.data)
         
      //   });
      // }
      // console.log(filteredTickets)
      // console.log(ticketNovos)
    ...
    async autoCloseTickets(tempo, mensagem) {
      if (this.autoFechamentoAtivo !== 'enabled') {
        ...
        await Promise.all(this.tickets.map(async (ticket) => {
          ...
          if (currentTimeInSeconds - lastMessageTimeInSeconds > tempo * 60 && ticket.status === 'open') {
